...
TRAIN_INPUT_DIR = 'data/Final/Train/'
VAL_INPUT_DIR = 'data/Final/Validate/'
...
opt = Adam(lr=learning_rate, decay=decay_rate, amsgrad=False)
model.compile(optimizer=opt, loss=dice_coef_loss, metrics=[iou, dice_coef])

callbacks = [ModelCheckpoint('unet_seg.hdf5', verbose=1, save_best_only=True), tf.keras.callbacks.EarlyStopping(patience=20, monitor='val_loss')]

history = model.fit(train_gen,
                    steps_per_epoch= LenghtTrainData / BATCH_SIZE,
                    epochs=EPOCHS,
                    callbacks=callbacks,
                    validation_data=test_gener,
                    validation_steps= LengthValData / BATCH_SIZE)
...
plt.savefig('outDat/Plots/Loss_new.png')
...
plt.savefig('outDat/Plots/Accuracy_new.png')
plt.show()
